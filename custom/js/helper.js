function renderClientList(data) {
    $.each(data, function(index, obj) {
        ...
        if (obj.Client.telegram_userid) {
            telegramButton =    `<div class="btn-group">      
                                    <button type="button" class="btn btn-outline-primary btn-sm" data-toggle="modal"
                                        ...
                                        data-clientname="${obj.Client.name}">${i18n.t('btn.telegram')}</button>
                                </div>`
        }
        ...
        const dnsEnabled = obj.Client.use_server_dns ? i18n.t('client.dns_enabled') : i18n.t('client.dns_disabled');
        ...
        let html = `<div class="col-sm-6 col-md-6 col-lg-4" id="client_${obj.Client.id}">
                        <div class="info-box">
                            ...
                            <div class="info-box-content" style="overflow: hidden">
                                <div class="btn-group">
                                    <a href="download?clientid=${obj.Client.id}" class="btn btn-outline-primary btn-sm">${i18n.t('btn.download')}</a>
                                </div>
                                <div class="btn-group">      
                                    <button type="button" class="btn btn-outline-primary btn-sm" data-toggle="modal"
                                        data-target="#modal_qr_client" data-clientid="${obj.Client.id}"
                                        data-clientname="${obj.Client.name}" ${obj.QRCode != "" ? '' : ' disabled'}>${i18n.t('btn.qr_code')}</button>
                                </div>
                                <div class="btn-group">      
                                    <button type="button" class="btn btn-outline-primary btn-sm" data-toggle="modal"
                                        data-target="#modal_email_client" data-clientid="${obj.Client.id}"
                                        data-clientname="${obj.Client.name}">${i18n.t('btn.email')}</button>
                                </div>
                                ${telegramButton}
                                <div class="btn-group">
                                    <button type="button" class="btn btn-outline-danger btn-sm">${i18n.t('btn.more')}</button>
                                    ...
                                    <div class="dropdown-menu" role="menu">
                                        ...
                                        data-clientname="${obj.Client.name}">${i18n.t('btn.edit')}</a>
                                        <a class="dropdown-item" href="#" data-toggle="modal"
                                        data-target="#modal_pause_client" data-clientid="${obj.Client.id}"
                                        data-clientname="${obj.Client.name}">${i18n.t('btn.disable')}</a>
                                        <a class="dropdown-item" href="#" data-toggle="modal"
                                        data-target="#modal_remove_client" data-clientid="${obj.Client.id}"
                                        data-clientname="${obj.Client.name}">${i18n.t('btn.delete')}</a>
                                    </div>
                                </div>
                                <hr>
                                ...
                                <span class="info-box-text"><strong>${i18n.t('client.ip_allocation')}</strong></span>`
                                + allocatedIpsHtml
                                + `<span class="info-box-text"><strong>${i18n.t('client.allowed_ips')}</strong></span>`
                                + allowedIpsHtml
                            +`</div>
                        </div>
                    </div>`
        ...
        let html = `<div class="col-sm-6 col-md-6 col-lg-4" id="user_${obj.username}">
                        <div class="info-box">
                            <div class="info-box-content">
                                <div class="btn-group">
                                     <button type="button" class="btn btn-outline-primary btn-sm" data-toggle="modal" data-target="#modal_edit_user" data-username="${obj.username}">${i18n.t('btn.edit')}</button>
                                </div>
                                <div class="btn-group">
                                    <button type="button" class="btn btn-outline-danger btn-sm" data-toggle="modal"
                                        data-target="#modal_remove_user" data-username="${obj.username}">${i18n.t('btn.delete')}</button>
                                </div>
                                <hr>
                                <span class="info-box-text"><i class="fas fa-user"></i> ${obj.username}</span>
                                <span class="info-box-text"><i class="fas fa-terminal"></i> ${obj.admin? i18n.t('users.admin'):i18n.t('users.admin')}</span>
                                </div>
                        </div>
                    </div>`
