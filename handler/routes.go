...
// BatchCreateUsers to create multiple users at once
func BatchCreateUsers(db store.IStore) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		usersData, ok := data["users"].([]interface{})
		if !ok || len(usersData) == 0 {
			return c.JSON(http.StatusBadRequest, jsonHTTPResponse{false, "Please provide users array"})
		}
		...
		for _, u := range usersData {
			...
			if !ok {
				failedUsers = append(failedUsers, "invalid user data")
				continue
			}

			username, ok := userMap["username"].(string)
			if !ok || username == "" {
				failedUsers = append(failedUsers, "missing username")
				continue
			}

			password, ok := userMap["password"].(string)
			if !ok || password == "" {
				failedUsers = append(failedUsers, username+": missing password")
				continue
			}

			admin, _ := userMap["admin"].(bool)

			if !usernameRegexp.MatchString(username) {
				failedUsers = append(failedUsers, username+": invalid username format")
				continue
			}

			_, err := db.GetUserByName(username)
			if err == nil {
				failedUsers = append(failedUsers, username+": username already exists")
				continue
			}

			hash, err := util.HashPassword(password)
			if err != nil {
				failedUsers = append(failedUsers, username+": "+err.Error())
				continue
			}
		...
		message := fmt.Sprintf("Successfully created %d user(s)", successCount)
		if len(failedUsers) > 0 {
			message += ". Failed: " + strings.Join(failedUsers, ", ")
		}
